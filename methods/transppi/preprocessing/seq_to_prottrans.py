...
def get_embeddings( seq_path, 
    ...
    start = time.time()
    batch = list()
    progress_bar = tqdm.tqdm(enumerate(seq_dict, 1), total=len(seq_dict))
    for seq_idx, (pdb_id, seq) in progress_bar:
        progress_bar.set_description(f'{pdb_id}, {len(seq)}')
...
def main():
    ...
    get_embeddings( seq_path, emb_path, model_dir, per_protein=per_protein, max_residues=1500, max_seq_len=1000, max_batch=100 )
